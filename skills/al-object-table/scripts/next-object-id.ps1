...
$app=Join-Path $Root 'app.json'
if(-not(Test-Path $app)){Write-Error "app.json not found at '$app'.";exit 1}
$j=Get-Content $app -Raw|ConvertFrom-Json
$ranges=@()
if($j.idRanges){$ranges=$j.idRanges|%{[pscustomobject]@{From=[int]$_.from;To=[int]$_.to}}}
elseif($j.idRange){$ranges=@([pscustomobject]@{From=[int]$j.idRange.from;To=[int]$j.idRange.to})}
...
Get-ChildItem $Root -Filter *.al -Recurse -File -EA SilentlyContinue|?{$_.FullName -notmatch '[\\/](\.alpackages|node_modules)[\\/]'}|%{
  $c=Get-Content $_.FullName -Raw -EA SilentlyContinue; if($c){foreach($m in [regex]::Matches($c,$pat)){[void]$used.Add([int]$m.Groups[2].Value)}}}
