...
Get-ChildItem (Join-Path $Path 'agents') -Filter *.md|%{$t=Get-Content $_.FullName -Raw
...
if(Test-Path $mp){$mj=Get-Content $mp -Raw|ConvertFrom-Json; $e=$mj.plugins|?{$_.name -eq $pj.name}|Select -First 1
  if($e.version -eq $pj.version){Ok "versions match $($pj.version)"}else{Bad "version mismatch"}}
