...
$chk=Join-Path $Path 'check-plugin-ready.ps1'; if(Test-Path $chk){& $chk -Path $Path|Out-Null; if($LASTEXITCODE -ne 0){Write-Host 'readiness failed';exit 1}}
if($Push){if(-not $Message){$Message="Release v$Version"}; Push-Location $Path; git add .|Out-Null; git commit -m $Message|Out-Null; git tag "v$Version"|Out-Null; git push|Out-Null; git push --tags|Out-Null; Pop-Location; Write-Host "Pushed v$Version"}
